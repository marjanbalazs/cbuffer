...
{
    cbuffer_handle_t cbuffer = malloc( sizeof (cbuffer_t));
...
int cbuffer_put(void* source_element, cbuffer_handle_t cbuffer)
{
    ...
    {
        void* ptr =  cbuffer->element_cbuffer +  cbuffer->write * cbuffer->sizeof_element;
...
void* cbuffer_get( cbuffer_handle_t cbuffer)
{
    ...
    {
        retptr = cbuffer->element_cbuffer + cbuffer->read * cbuffer->sizeof_element;
        cbuffer->full = 0;
        cbuffer->read++;
    }
